...
if __name__ == '__main__':
    ...
    target_feature_pkl = "/path/to/target/model/trained/using/USPS/feature_extractor.pkl"
    target_cls_pkl = "/path/to/target/model/trained/using/USPS/cls.pkl"
    aux_feature_pkl = "/path/to/auxiliary/classifier/trained/using/MNIST/feature_extractor.pkl"
    aux_cls_pkl = "/path/to/auxiliary/classifier/trained/using/MNIST/cls.pkl"
    h = 32
    w = 32
    lr = 0.01
    momentum = 0.9
    ...
    init = Init(seed=9970, log_root_dir=root_dir, sep=True,
                backup_filename=__file__, tensorboard=True, comment=f'MNIST DATTACKB M2U BLACK')
    ...
    source_train_ds = mnist_train_ds
    source_test_ds = mnist_test_ds

    target_train_ds = usps_train_ds
    target_test_ds = usps_test_ds
    ...
    # for evaluate
    train_dl = DataLoader(dataset=target_train_ds, batch_size=32,
                          shuffle=False, num_workers=data_workers, drop_last=False)
    # for attack
    test_dl = DataLoader(dataset=source_train_ds, batch_size=32,
                         shuffle=False, num_workers=data_workers, drop_last=False)
    ...
    class Classifier(nn.Module):
        def __init__(self) -> None:
            ...
            self.cls = CLS(8192, class_num, bottle_neck_dim=50)
    ...
    class PowerAmplification(nn.Module):
        def __init__(self, in_features: int, alpha: float = None, device=None, dtype=None) -> None:
            super(PowerAmplification, self).__init__()
            factory_kwargs = {'device': device, 'dtype': dtype}
            self.in_features = in_features
            self.epsion=0.000000001
    ...
    myamplification = PowerAmplification(
        class_num, 1/6).train(False).to(output_device)
    ...
    target_classifier.feature_extractor.load_state_dict(torch.load(
        open(target_feature_pkl, 'rb'), map_location=output_device))

    assert os.path.exists(target_cls_pkl)
    target_classifier.cls.load_state_dict(torch.load(
        open(target_cls_pkl, 'rb'), map_location=output_device))

    assert os.path.exists(aux_feature_pkl)
    aux_classifier.feature_extractor.load_state_dict(torch.load(
        open(aux_feature_pkl, 'rb'), map_location=output_device))

    assert os.path.exists(aux_cls_pkl)
    aux_classifier.cls.load_state_dict(torch.load(
        open(aux_cls_pkl, 'rb'), map_location=output_device))
    ...
    optimizer_auxcls = OptimWithSheduler(optim.SGD(aux_classifier.parameters(
    ), lr=lr/10.0, weight_decay=weight_decay, momentum=momentum, nesterov=True), scheduler)
    ...
    for epoch_id in tqdm(range(1, train_epoches+1), desc='Total Epoch'):
        for i, ((im_source, label_source), (im_target, label_target)) in enumerate(tqdm(zip(source_train_dl, target_train_dl), desc=f'epoch {epoch_id}', total=min(len(source_train_dl), len(target_train_dl)))):
            ...
            target_after_softmax = myamplification.forward(
                target_after_softmax)
        ...
        if epoch_id % log_epoch == 0:
            ...
            save_image2(
                im_source.detach(), f'{log_dir}/input/{epoch_id}.png')
            save_image2(
                rim_source.detach(), f'{log_dir}/output/{epoch_id}.png')
            with open(os.path.join(model_dir, f'aux_feature_extractor_{epoch_id}.pkl'), 'wb') as f:
            ...
            with torch.no_grad():
                ...
                r = 0
                mmdloss = 0
                for i, ((im_source, label_source), (im_target, label_target)) in enumerate(tqdm(zip(source_train_dl, target_train_dl), desc=f'MMD', total=min(len(source_train_dl), len(target_train_dl)))):
                    r += 1
                    im_source = im_source.to(output_device)
                    label_source = label_source.to(output_device)
                    im_target = im_target.to(output_device)
